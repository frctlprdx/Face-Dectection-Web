// src/components/WebcamModal.tsx
import React, { useRef, useState, useEffect } from 'react';
// Pastikan path MessageDisplay benar relatif terhadap WebcamModal.tsx
import MessageDisplay from './MessageDisplay';

interface WebcamModalProps {
  isOpen: boolean;
  onClose: () => void;
  onPhotosCaptured: (photos: { index: number; dataUrl: string | null; status: 'pending' | 'taken' }[]) => void;
  currentPhotoIndex: number;
  setCurrentPhotoIndex: React.Dispatch<React.SetStateAction<number>>;
  photoInstructions: string[];
  capturedPhotos: { index: number; dataUrl: string | null; status: 'pending' | 'taken' }[];
  setCapturedPhotos: React.Dispatch<React.SetStateAction<{ index: number; dataUrl: string | null; status: 'pending' | 'taken' }[]>>;
  setMessage: React.Dispatch<React.SetStateAction<{ text: string; type: 'success' | 'error' | 'info' }>>;
}

const WebcamModal: React.FC<WebcamModalProps> = ({
  isOpen,
  onClose,
  onPhotosCaptured,
  currentPhotoIndex,
  setCurrentPhotoIndex,
  photoInstructions,
  capturedPhotos,
  setCapturedPhotos,
  setMessage,
}) => {
  const videoRef = useRef<HTMLVideoElement>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [isWebcamActive, setIsWebcamActive] = useState<boolean>(false);

  useEffect(() => {
    if (isOpen) {
      startWebcam();
    } else {
      stopWebcam();
      // Reset currentPhotoIndex saat modal ditutup, agar bisa mulai dari 0 lagi
      setCurrentPhotoIndex(0);
    }
    return () => {
      stopWebcam();
    };
  }, [isOpen]);

  useEffect(() => {
    if (isOpen && isWebcamActive && currentPhotoIndex < photoInstructions.length) {
      setMessage({ text: photoInstructions[currentPhotoIndex], type: 'info' });
    }
  }, [currentPhotoIndex, isOpen, isWebcamActive, photoInstructions, setMessage]);

  const startWebcam = async () => {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ video: true });
      if (videoRef.current) {
        videoRef.current.srcObject = stream;
        videoRef.current.onloadedmetadata = () => {
          videoRef.current?.play();
          setIsWebcamActive(true);
        };
      }
    } catch (err) {
      console.error("Error accessing webcam: ", err);
      setMessage({ text: "Tidak bisa mengakses webcam. Pastikan Anda mengizinkan akses kamera.", type: "error" });
      setIsWebcamActive(false);
      onClose();
    }
  };

  const stopWebcam = () => {
    if (videoRef.current && videoRef.current.srcObject) {
      const stream = videoRef.current.srcObject as MediaStream;
      const tracks = stream.getTracks();
      tracks.forEach(track => track.stop());
      videoRef.current.srcObject = null;
      setIsWebcamActive(false);
    }
  };

  const capturePhoto = () => {
    if (videoRef.current && canvasRef.current && isWebcamActive) {
      const video = videoRef.current;
      const canvas = canvasRef.current;
      const context = canvas.getContext('2d');

      if (!context) {
        setMessage({ text: 'Gagal mendapatkan konteks canvas.', type: 'error' });
        return;
      }

      // Pastikan ukuran canvas sesuai dengan video
      canvas.width = video.videoWidth;
      canvas.height = video.videoHeight;
      context.drawImage(video, 0, 0, canvas.width, canvas.height);

      const newImageDataUrl = canvas.toDataURL('image/jpeg', 0.9);

      const updatedPhotos = [...capturedPhotos];
      updatedPhotos[currentPhotoIndex] = { ...updatedPhotos[currentPhotoIndex], dataUrl: newImageDataUrl, status: 'taken' };
      setCapturedPhotos(updatedPhotos);

      if (currentPhotoIndex < photoInstructions.length - 1) {
        setCurrentPhotoIndex(prevIndex => prevIndex + 1);
        // Message updated by useEffect for currentPhotoIndex change
      } else {
        setMessage({ text: 'Semua foto berhasil diambil. Silakan lengkapi data diri.', type: 'success' });
        onPhotosCaptured(updatedPhotos);
        onClose(); // Tutup modal
      }
    } else {
      setMessage({ text: 'Kamera belum aktif atau elemen tidak siap.', type: 'error' });
    }
  };

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full flex flex-col items-center relative"> {/* Added relative for close button */}
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-3 right-3 text-gray-500 hover:text-gray-700 text-3xl font-bold p-1 leading-none rounded-full bg-gray-100 hover:bg-gray-200"
          aria-label="Close modal"
        >
          &times;
        </button>

        <h3 className="text-2xl font-bold text-gray-800 mb-4">Pendaftaran Wajah</h3>
        
        {isWebcamActive && currentPhotoIndex < photoInstructions.length && (
          <p className="text-lg text-gray-700 font-medium mb-4 text-center animate-pulse">
            {photoInstructions[currentPhotoIndex]}
          </p>
        )}

        {/* Kontainer untuk webcam bulat */}
        <div className="relative w-64 h-64 rounded-full overflow-hidden bg-gray-200 border-4 border-blue-500 mb-6 flex items-center justify-center">
          {!isWebcamActive && (
            <span className="text-gray-500 text-sm">Memuat kamera...</span>
          )}
          <video
            ref={videoRef}
            autoPlay
            playsInline
            className="absolute top-0 left-0 w-full h-full object-cover transform scale-x-[-1]" // Mirroring webcam
          ></video>
          <canvas ref={canvasRef} style={{ display: 'none' }}></canvas>
        </div>

        <button
          onClick={capturePhoto}
          disabled={!isWebcamActive || currentPhotoIndex >= photoInstructions.length}
          className="px-8 py-3 bg-green-600 text-white font-semibold rounded-full shadow-lg hover:bg-green-700 focus:outline-none focus:ring-4 focus:ring-green-500 focus:ring-opacity-75 disabled:opacity-50 disabled:cursor-not-allowed transition duration-300 ease-in-out"
        >
          {currentPhotoIndex < photoInstructions.length
            ? `Ambil Foto (${currentPhotoIndex + 1}/${photoInstructions.length})`
            : 'Selesai Mengambil Foto'}
        </button>

        {/* Removed 'Batalkan' button, replaced by close button on top right for modal standard UX */}
        {/* <button
          onClick={() => {
            stopWebcam();
            onClose();
          }}
          className="mt-4 text-gray-600 hover:text-gray-800 text-sm"
        >
          Batalkan
        </button> */}
      </div>
    </div>
  );
};

export default WebcamModal;