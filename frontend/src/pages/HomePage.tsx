...
const HomePage: React.FC = () => {
    ...
    const fetchAttendances = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_LARAVEL_API_URL}/attendances`,
          {
            headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      {isSidebarOpen && (
        <>
          <div className="fixed inset-0 bg-transparent z-20" onClick={() => setIsSidebarOpen(false)}></div>
          <div className="fixed top-0 left-0 w-64 h-full bg-white shadow-xl z-30 transform transition-transform duration-300 ease-in-out">
            ...
            <nav className="flex flex-col p-4 space-y-2">
              <Link to="/register-face" onClick={() => setIsSidebarOpen(false)} className="block px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-md">
                Daftarkan Wajah
              </Link>
              <Link to="/recognize-face" onClick={() => setIsSidebarOpen(false)} className="block px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-md">
                Presensi Harian
              </Link>
            </nav>
          </div>
      ...
      <main className="flex-grow p-6">
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-3xl font-bold text-gray-800 mb-6 border-b pb-4">History Presensi</h2>
          <div className="space-y-4">
            {attendances.length === 0 ? (
              <p className="text-gray-600">Belum ada data presensi.</p>
            ) : (
              attendances.map((entry) => (
                <div key={entry.id} className="border border-gray-200 rounded-lg p-4 bg-gray-50 flex justify-between items-center">
                  <div>
                    ...
                    {entry.status === "absent" ? (
                      ...
                      <>
                        <p className="text-gray-700">Waktu Masuk: {entry.attendance_time ? new Date(entry.attendance_time).toLocaleTimeString("id-ID") : "-"}</p>
                      </>
                    )}
                  </div>
                  <span
                  ...
                  >
                    {entry.status === "present" ? "Hadir" : entry.status === "absent" ? "Absen" : "Pending"}
                  </span>
                </div>
              ))
            )}
