...
const AuthPage: React.FC = () => {
  ...
  const [registerRole, setRegisterRole] = useState<string>(""); // Role yang dipilih

  // State untuk pesan status
  const [message, setMessage] = useState<Message>({
    text: "Silakan masuk atau daftar akun baru.",
    type: "info",
  });
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok && data.token) {
        // Asumsi Laravel mengembalikan 'token' dan 'user'
        setMessage({
        ...
        setMessage({
          text: data.message || "Login gagal. Periksa kembali kredensial Anda.",
          type: "error",
        });
      }
    } catch (error) {
      ...
      setMessage({
        text: "Terjadi kesalahan jaringan atau server tidak merespons saat login.",
        type: "error",
      });
    }
  };
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(`${LARAVEL_API_URL}/register`, {
        // Asumsi ada endpoint /register di Laravel
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
      ...
      if (response.ok) {
        setMessage({
          text: data.message || "Pendaftaran berhasil! Silakan login.",
          type: "success",
        });
      ...
      setMessage({
        text: "Terjadi kesalahan jaringan atau server tidak merespons saat pendaftaran.",
        type: "error",
      });
    }
  };
        ...
        {formType === "register" ? (
          <>
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full bg-indigo-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-indigo-700 focus:outline-none focus:shadow-outline focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-75 transition duration-300 ease-in-out cursor-pointer"
              >
                Daftar
              </button>
            </form>
